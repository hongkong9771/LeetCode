...
区间dp求解是由小区间合并成大区间的，也就是我们要从长度最短的区间开始解决子问题，当区间长度为1时，我们不难得出其最小涂色次数为1，
当区间长度大于1时，最小染色次数为 左区间最小次数+右区间最小次数，因此我们需要枚举该区间每个断点所得到的最小染色次数最小值。
特别的，当a[i] ==a[j] 时，可以由min（dp[i][j-1],dp[i+1][j]）进行转移过来，因为涂一次可以涂一整个区间，l或者r的那一次染色可以拿来帮助另一个染色。
